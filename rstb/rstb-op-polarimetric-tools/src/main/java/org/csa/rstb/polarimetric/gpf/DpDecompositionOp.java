...
/**
 * Generate scattering power components for dual-pol product based on decomposition
 * Note: 1. This operator computes dual-pol scattering power components from dual-pol SAR data using decompostion approach.
 ...
 * Reference:
 * [1] Verma, A., Bhattacharya, A., Dey, S., Marino, A., and Gamba, P., 2024.
...
@OperatorMetadata(alias = "DualPol-Powers-Decomposition",
...
public final class DpDecompositionOp extends Operator implements DualPolProcessor {
    ...
    private final static String DP_POWERS_PU = "Pu";
    private final static String DP_ALPHA_S= "Alpha_S";
            ...
            try {
                ...
                for (int y = y0; y < maxY; ++y) {
                    ...
                    for (int x = x0; x < maxX; ++x) {
                        ...
                        // Normalize Stokes parameters using 95th percentiles
                        ...
                        double dop = (EigenVal[0] - EigenVal[1])/(EigenVal[0] + EigenVal[1]);
                        double beta = EigenVal[0]/(EigenVal[0] + EigenVal[1]);
                        ...
                        // Computing dual-pol alpha_S
                        double alpha1 = Math.atan2(dpRBI, 1 - dpRBI);
                        alpha1 = Math.toDegrees(alpha1);

                        double alpha2 = Math.atan2(1 - dpRSI, dpRSI);
                        alpha2 = Math.toDegrees(alpha2);

                        double alpha_dp = (alpha1 + alpha2) / 2;

                        double alpha_dp_rad = Math.toRadians(2 * alpha_dp);
                        double cos_a = Math.cos(alpha_dp_rad);

                        //Power computation (Decomposition)
                        double Pu = (c11_dB > NESZ) ? (1 - dop) * g0 : (1 - beta) * g0;
                        double Pd = (c11_dB > NESZ) ? 0.5 * dop * g0 * (1 - cos_a) : 0.5 * beta * g0 * (1 - cos_a);
                        double Ps = (c11_dB > NESZ) ? 0.5 * dop * g0 * (1 + cos_a) : 0.5 * beta * g0 * (1 + cos_a);

                        for (final TileData tileData : tileDataList) {
                            // Can add more indices here
                            if (tileData.bandName.contains(DP_POWERS_PU)) {
                                tileData.dataBuffer.setElemFloatAt(tgtIdx, (float) Pu);
                            }
                            ...
                            else if (tileData.bandName.contains(DP_ALPHA_S)) {
                                tileData.dataBuffer.setElemFloatAt(tgtIdx, (float) alpha_dp);
                            }
                        }
                    }
                }

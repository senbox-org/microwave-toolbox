...
import static eu.esa.sar.io.sentinel1.Sentinel1Directory.sentinelDateFormat;
...
public class CapellaProductDirectory extends JSONProductDirectory {
    ...
    protected void addAbstractedMetadataHeader(final MetadataElement root) {
        ...
        final MetadataAttribute attribute = imageGeometry.getAttribute("delta_line_time");
        if (attribute != null) {
            final double delta_line_time = imageGeometry.getAttributeDouble("delta_line_time");
            final double lastTime = firstTime + (height - 1) * delta_line_time;

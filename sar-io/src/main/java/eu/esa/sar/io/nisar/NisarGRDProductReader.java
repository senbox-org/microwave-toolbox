...
public class NisarGRDProductReader extends SARReader {
     ...
     *                     implementations
     */
    public NisarGRDProductReader(final ProductReaderPlugIn readerPlugIn) {
        super(readerPlugIn);
    }
    ...
    private static void addGeoCodingFromMetadata(final Product product, Map<String, String> netcdfFile) {
        ...
        double[] firstNear = convertStringToDoubleArray(netcdfFile.get(NisarXConstants.FIRST_NEAR.toUpperCase()));
        double[] firstFar = convertStringToDoubleArray(netcdfFile.get(NisarXConstants.FIRST_FAR.toUpperCase()));
        double[] lastNear = convertStringToDoubleArray(netcdfFile.get(NisarXConstants.LAST_NEAR.toUpperCase()));
        double[] lastFar = convertStringToDoubleArray(netcdfFile.get(NisarXConstants.LAST_FAR.toUpperCase()));
        ...
        AbstractMetadata.setAttribute(absRoot, AbstractMetadata.range_spacing,
                Double.valueOf(netcdfFile.get(NisarXConstants.RANGE_SPACING.toUpperCase())));
        AbstractMetadata.setAttribute(absRoot, AbstractMetadata.azimuth_spacing,
                Double.valueOf(netcdfFile.get(NisarXConstants.AZIMUTH_SPACING.toUpperCase())));
    ...
    private static double[] convertStringToDoubleArrayBySpace(String string) {
        return Arrays.stream(string.replace("\n", " ").replaceAll("\\s+", " ").
                ...
                trim().split(" ")).mapToDouble(Double::parseDouble).toArray();
    }
        ...
        try {
            ...
            for (int i = 0; i < tempNetcdfFile.getRootIFD().getTIFFFields().length; i++) {
                ...
                if (tiffFields.getType() == 2 && tiffFields.getData() != null &&
                        ...
                        ((String[]) tiffFields.getData())[0].startsWith(NisarXConstants.GDALMETADATA)) {
            ...
            if (document == null || document.getFirstChild() == null ||
                ...
                close();
                throw new IllegalFileFormatException("No NISAR metadata variables found which could\n" +
            ...
            final String productType = this.tiffFeilds.get(
                    NisarXConstants.PRODUCT_TYPE.toUpperCase());

            final int rasterWidth = Integer.parseInt(this.tiffFeilds.get(
                    NisarXConstants.NUM_SAMPLES_PER_LINE.toUpperCase()));

            final int rasterHeight = Integer.parseInt(this.tiffFeilds.get(
                    NisarXConstants.NUM_OUTPUT_LINES.toUpperCase()));
            ...
            description.append(this.tiffFeilds.get(NisarXConstants.PRODUCT.toUpperCase())).append(" - ");
            description.append(this.tiffFeilds.get(NisarXConstants.PRODUCT_TYPE.toUpperCase())).append(" - ");
            description.append(this.tiffFeilds.get(NisarXConstants.SPH_DESCRIPTOR.toUpperCase())).append(" - ");
            description.append(this.tiffFeilds.get(NisarXConstants.MISSION.toUpperCase()));
            product.setDescription(description.toString());
            product.setStartTime(ProductData.UTC.parse(this.tiffFeilds.get(
                    NisarXConstants.ACQUISITION_START_UTC.toUpperCase()), standardDateFormat));

            product.setEndTime(ProductData.UTC.parse(this.tiffFeilds.get(
                    NisarXConstants.ACQUISITION_END_UTC.toUpperCase()), standardDateFormat));
    ...
    private void addAbstractedMetadataHeader(MetadataElement root) {
        ...
        try {
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.PRODUCT, tiffFeilds.get(
                    NisarXConstants.PRODUCT.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.PRODUCT_TYPE, tiffFeilds.get(
                    NisarXConstants.PRODUCT_TYPE.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.SPH_DESCRIPTOR, tiffFeilds.get(
                    NisarXConstants.SPH_DESCRIPTOR.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.MISSION, Missions.NISAR);

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.ACQUISITION_MODE, tiffFeilds.get(
                    NisarXConstants.ACQUISITION_MODE.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.antenna_pointing, tiffFeilds.get(
                    NisarXConstants.ANTENNA_POINTING.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.BEAMS,
                    NisarXConstants.BEAMS_DEFAULT_VALUE);

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.PROC_TIME, ProductData.UTC.parse(
                    tiffFeilds.get(NisarXConstants.PROC_TIME_UTC.toUpperCase()), standardDateFormat));


            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.ProcessingSystemIdentifier,
                    tiffFeilds.get(NisarXConstants.PROCESSING_SYSTEM_IDENTIFIER.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.CYCLE,
                    Integer.valueOf(tiffFeilds.get(NisarXConstants.CYCLE.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.REL_ORBIT,
                    Integer.valueOf(tiffFeilds.get(NisarXConstants.REL_ORBIT.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.ABS_ORBIT,
                    Integer.valueOf(tiffFeilds.get(NisarXConstants.ABS_ORBIT.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.incidence_near,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.INCIDENCE_NEAR.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.incidence_far,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.INCIDENCE_FAR.toUpperCase())));
            ...
            String geoRefSystem = NisarXConstants.GEO_REFERENCE_SYSTEM_DEFAULT_VALUE;
            if (tiffFeilds.get(NisarXConstants.GEO_REFERENCE_SYSTEM.toUpperCase()) != null) {
                geoRefSystem = tiffFeilds.get(NisarXConstants.GEO_REFERENCE_SYSTEM.toUpperCase());
            }
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.first_line_time, ProductData.UTC.parse(
                    tiffFeilds.get(NisarXConstants.FIRST_LINE_TIME.toUpperCase()), standardDateFormat));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.last_line_time, ProductData.UTC.parse(
                    tiffFeilds.get(NisarXConstants.LAST_LINE_TIME.toUpperCase()), standardDateFormat));

            double[] firstNear = convertStringToDoubleArray(tiffFeilds.get(NisarXConstants.FIRST_NEAR.toUpperCase()));
            ...
            double[] firstFar = convertStringToDoubleArray(tiffFeilds.get(NisarXConstants.FIRST_FAR.toUpperCase()));
            ...
            double[] lastNear = convertStringToDoubleArray(tiffFeilds.get(NisarXConstants.LAST_NEAR.toUpperCase()));
            ...
            double[] lastFar = convertStringToDoubleArray(tiffFeilds.get(NisarXConstants.LAST_FAR.toUpperCase()));
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.PASS, tiffFeilds.get(
                    NisarXConstants.PASS.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.SAMPLE_TYPE, getSampleType());
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.mds1_tx_rx_polar, tiffFeilds.get(
                    NisarXConstants.MDS1_TX_RX_POLAR.toUpperCase()));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.azimuth_looks, Float.valueOf(
                    tiffFeilds.get(NisarXConstants.AZIMUTH_LOOKS.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.range_looks, Float.valueOf(
                    tiffFeilds.get(NisarXConstants.RANGE_LOOKS.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.range_spacing, Float.valueOf(
                    tiffFeilds.get(NisarXConstants.RANGE_SPACING.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.azimuth_spacing, Float.valueOf(
                    tiffFeilds.get(NisarXConstants.AZIMUTH_SPACING.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.pulse_repetition_frequency, Float.valueOf(
                    tiffFeilds.get(NisarXConstants.PULSE_REPETITION_FREQUENCY.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.radar_frequency, Double.valueOf(
                    tiffFeilds.get(NisarXConstants.RADAR_FREQUENCY.toUpperCase())) / Constants.oneMillion);

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.line_time_interval, Double.valueOf(
                    tiffFeilds.get(NisarXConstants.LINE_TIME_INTERVAL.toUpperCase())));

            final int rasterWidth = Integer.parseInt(tiffFeilds.get(NisarXConstants.NUM_SAMPLES_PER_LINE.toUpperCase()));
            final int rasterHeight = Integer.parseInt(tiffFeilds.get(NisarXConstants.NUM_OUTPUT_LINES.toUpperCase()));
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.num_output_lines, Integer.valueOf(
                    tiffFeilds.get(NisarXConstants.NUM_OUTPUT_LINES.toUpperCase())));

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.num_samples_per_line, Integer.valueOf(
                    tiffFeilds.get(NisarXConstants.NUM_SAMPLES_PER_LINE.toUpperCase())));
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.avg_scene_height, Double.valueOf(
                    tiffFeilds.get(NisarXConstants.AVG_SCENE_HEIGHT.toUpperCase())));
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.slant_range_to_first_pixel,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.SLANT_RANGE_TO_FIRST_PIXEL.toUpperCase())));

            int antElevCorrFlag = NisarXConstants.ANT_ELEV_CORR_FLAG_DEFAULT_VALUE;
            if (tiffFeilds.get(NisarXConstants.ANT_ELEV_CORR_FLAG.toUpperCase()) != null) {
                antElevCorrFlag = Integer.valueOf(tiffFeilds.get(NisarXConstants.ANT_ELEV_CORR_FLAG.toUpperCase()));
            }
            ...
            if (tiffFeilds.get(NisarXConstants.RANGE_SPREAD_COMP_FLAG.toUpperCase()) != null) {
                rangeSpreadCompFlag = Integer.valueOf(tiffFeilds.get(NisarXConstants.RANGE_SPREAD_COMP_FLAG.toUpperCase()));
            }
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.calibration_factor,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.CALIBRATION_FACTOR.toUpperCase())));
            ...
            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.range_sampling_rate,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.RANGE_SAMPLING_RATE.toUpperCase())) / 1e6);

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.range_bandwidth,
                    Double.valueOf(tiffFeilds.get(NisarXConstants.RANGE_BANDWIDTH.toUpperCase())) / 1e6);

            AbstractMetadata.setAttribute(absRoot, AbstractMetadata.azimuth_bandwidth, Double.valueOf(
                    tiffFeilds.get(NisarXConstants.AZIMUTH_BANDWIDTH.toUpperCase())));
    ...
    private void addSRGRCoefficients(final MetadataElement absRoot) {
        ...
        try {
            utcTime = ProductData.UTC.parse(tiffFeilds.get(NisarXConstants.GRSR_ZERO_DOPPLER_TIME.toUpperCase()),
                    standardDateFormat);
        ...
        AbstractMetadata.setAttribute(srgrListElem, AbstractMetadata.ground_range_origin,
                Double.valueOf(tiffFeilds.get(NisarXConstants.GRSR_GROUND_RANGE_ORIGIN.toUpperCase())));

        final String[] coeffStrArray = convertDateStringToStringArrayBySpace(tiffFeilds.get(
                NisarXConstants.GRSR_COEFFICIENTS.toUpperCase()));
    ...
    private void addOrbitStateVectors(final MetadataElement absRoot) {
        ...
        try {
            ...
            String[] stateVectorTime = convertDateStringToStringArray(tiffFeilds.get(NisarXConstants.STATE_VECTOR_TIME.toUpperCase()));
            final int numPoints = stateVectorTime.length;
            final double[] satellitePositionX = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_X_POS.toUpperCase()));

            final double[] satellitePositionY = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_Y_POS.toUpperCase()));

            final double[] satellitePositionZ = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_Z_POS.toUpperCase()));

            final double[] satelliteVelocityX = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_X_VEL.toUpperCase()));

            final double[] satelliteVelocityY = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_Y_VEL.toUpperCase()));

            final double[] satelliteVelocityZ = convertStringToDoubleArrayBySpace(tiffFeilds.get(
                    NisarXConstants.ORBIT_VECTOR_N_Z_VEL.toUpperCase()));
    ...
    private void addDopplerCentroidCoefficients() {
        ...
        final ProductData.UTC utcTime;
        try {
            utcTime = ProductData.UTC.parse(convertDateStringToStringArray(tiffFeilds.get(
                    NisarXConstants.DC_ESTIMATE_TIME_UTC.toUpperCase()))[0], standardDateFormat);
        ...
        AbstractMetadata.setAttribute(dopplerListElem, AbstractMetadata.slant_range_time,
                Double.valueOf(tiffFeilds.get(NisarXConstants.DC_REFERENCE_PIXEL_TIME.toUpperCase())) * 1e9);

        int dimensionColumn = Integer.valueOf(tiffFeilds.get(NisarXConstants.DC_ESTIMATE_POLY_ORDER.toUpperCase())) + 1;

        String[] coefValueS = convertDateStringToStringArrayBySpace(
                tiffFeilds.get(NisarXConstants.DC_ESTIMATE_COEFFS.toUpperCase()));
    ...
    private String getSampleType() {
        if (NisarXConstants.SLC.equalsIgnoreCase(tiffFeilds.get(NisarXConstants.SPH_DESCRIPTOR.toUpperCase()))) {
            isComplex = true;
    ...
    private void addBandsToProduct() {
        try {
            ...
            try (final InputStream inStream = new BufferedInputStream(new FileInputStream(inputFile))) {
                final ImageInputStream imgStream =
                        ImageIOFile.createImageInputStream(inStream, new Dimension(rasterWidth, rasterHeight));

                final ImageIOFile img = new ImageIOFile(name, imgStream,
                        GeoTiffUtils.getTiffIIOReader(imgStream), inputFile);

                String polarization = tiffFeilds.get(NisarXConstants.MDS1_TX_RX_POLAR.toUpperCase());
    ...
    private void addTiePointGridsToProduct() {
        ...
        final double slantRangeToFirstPixel = Double.parseDouble(
                tiffFeilds.get(NisarXConstants.SLANT_RANGE_TO_FIRST_PIXEL.toUpperCase())); // in m

        final double rangeSpacing = Double.parseDouble(tiffFeilds.get(NisarXConstants.RANGE_SPACING.toUpperCase())); // in m

        final boolean srgrFlag = tiffFeilds.get(
                NisarXConstants.SPH_DESCRIPTOR.toUpperCase()).equalsIgnoreCase(NisarXConstants.GRD);

        // get scene center latitude
        String coordCenter = tiffFeilds.get(NisarXConstants.COORD_CENTER.toUpperCase());
        double sceneCenterLatitude = Double.parseDouble(
                coordCenter.substring(1, coordCenter.length() - 1).split(",")[2]); // in deg [3]

        final double nearRangeIncidenceAngle = Double.parseDouble(
                tiffFeilds.get(NisarXConstants.INCIDENCE_NEAR.toUpperCase()));
    ...
    private void addSlantRangeTime(final Product product) {
        ...
        String[] coeffArray = convertDateStringToStringArrayBySpace(tiffFeilds.get(
                NisarXConstants.GRSR_COEFFICIENTS.toUpperCase()));

        final CoefList coefList = new CoefList();

        try {
            coefList.utcSeconds = ProductData.UTC.parse(this.tiffFeilds.get(
                    NisarXConstants.GRSR_ZERO_DOPPLER_TIME.toUpperCase()), standardDateFormat).getMJD() * 24 * 3600;

            coefList.grOrigin = Double.valueOf(tiffFeilds.get(NisarXConstants.GRSR_GROUND_RANGE_ORIGIN.toUpperCase()));
    ...
    private void setRangeDist(MetadataElement absRoot, List<CoefList> segmentsArray, int gridWidth,
                              int gridHeight, int subSamplingX, float[] rangeDist) throws ParseException {

        final double lineTimeInterval = Double.parseDouble(this.tiffFeilds.get(
                NisarXConstants.LINE_TIME_INTERVAL.toUpperCase()));

        final double startSeconds = ProductData.UTC.parse(this.tiffFeilds.get(
                NisarXConstants.FIRST_LINE_TIME.toUpperCase()), standardDateFormat).getMJD() * 24 * 3600;
    ...
    protected void readBandRasterDataImpl(int sourceOffsetX, int sourceOffsetY, int sourceWidth, int sourceHeight,
        ...
        if (bandInfo != null && bandInfo.img != null) {
            if (tiffFeilds.get(NisarXConstants.PASS.toUpperCase()).equalsIgnoreCase(NisarXConstants.ASCENDING)) {

...
| Elongated bands / subtle phase breaks along burst seams, strongest over part of the range swath, at several seams | The cross-acquisition annotation-error difference carries **per-burst range structure** — a smooth, range-varying step of several radians per seam that wraps along the swath (so it can look strong at near range and invisible mid-swath). With `subtractResidualRamp=true` the Interferogram step now also **measures each seam's residual discontinuity directly and subtracts it** (log: `GSLC seam steps (pair N): [s0 ... rad @near/mid/far]`); across-seam differencing cannot absorb deformation or atmosphere, which are continuous across seams. Seams inside heavily decorrelated areas are left untouched (unmeasurable — and invisible there anyway). |

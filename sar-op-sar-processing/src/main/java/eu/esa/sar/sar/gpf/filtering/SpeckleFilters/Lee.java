 ...
 * Lee Speckle Filter
 */
public class Lee implements SpeckleFilter {
    ...
    private final boolean estimateENL;
    private final double enl;

    public Lee(final Operator op, final Product srcProduct, final Product trgProduct, final int windowSizeX,
    ...
    public void computeTile(Band targetBand, Tile targetTile, ProgressMonitor pm) {
        ...
        try {
            ...
            OperatorUtils.catchOperatorException("Lee", e);
        } finally {
            pm.done();
        }
    }
    ...
    public double[][] performFiltering(
        ...
        for (int y = y0; y < yMax; ++y) {
            ...
            for (int x = x0; x < xMax; ++x) {
                ...
                if (numSamples > 0) {
                    filteredTile[yy][xx] = getLeeValue(neighborValues, numSamples, noDataValue, cu, cu2);
                } else {

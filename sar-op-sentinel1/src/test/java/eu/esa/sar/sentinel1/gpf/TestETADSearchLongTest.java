...
/**
 * Live Copernicus Data Space ETAD search + download tests, split out of {@link TestETADSearch}
 * (which keeps the fast, offline unit tests): each of these performs a real catalogue search
 * and the GRD/SLC ones download a full ETAD product (~13-45 s + network + credentials), so they
 * are gated per the long-test convention ({@code -Denable.long.tests=true}) instead of taxing
 * every default {@code mvn test} run.
 */
@STTM("SNAP-3707")
@RunWith(LongTestRunner.class)
public class TestETADSearchLongTest {
